"""Build an adult anatomical character prototype from licensed MakeHuman data."""
...
try:
 ...
 neck=center('joint-neck');head=center('joint-head');mouth=center('joint-mouth');eye_level=(center('joint-l-eye').z+center('joint-r-eye').z)/2;hands=[center('joint-l-hand'),center('joint-r-hand')]
 for face in body.data.polygons:
  c=sum((body.data.vertices[i].co for i in face.vertices),Vector())/len(face.vertices)
  face.material_index=0 if c.z>neck.z-.025 or min((c-h).length for h in hands)<.105 else 3 if c.z<.13 else 2 if c.z<.86 else 1
 ...
 for face in groups['helper-tights']:
  ...
  if .84<c.z<neck.z-.035 and abs(c.x)<.56 and min((c-h).length for h in hands)>.11:garment.append(face)
 ...
 for face in groups['helper-hair']:
  c=sum((points[i] for i,_ in face),Vector())/len(face)
  if c.z>eye_level+.035 or c.y>head.y+.025:scalp_faces.append(face)
