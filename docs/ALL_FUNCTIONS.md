...
| 11 | **我的** | （待建）`me.html` | 头像 + 会员标识 + 余额 + 入口（背包/图鉴/订阅/钱包/交易记录/隐私/注销/安全/关于）+ 实名/年龄核验状态 | ❌ 缺失（仅 profile.html 个人主页部分覆盖） |
...
| P | 缺口 | 说明 |
|---|---|---|
| P0 | 角色详情页 character-detail.html | 双按钮（进入 TA 的世界 / 带 TA 回到身边）+ 资格进度 |
| P0 | 我的页 me.html | 5 入口（背包/图鉴/订阅/钱包/关于）+ 实名/年龄核验 |
