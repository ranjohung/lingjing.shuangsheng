...
| 7 | **创作中心** | `creator-center.html` | 功能区 5 卡：编辑我的作品 + 小说辅助 + 上传即生成 + 上架与定价 + 创建新作品；工作台七宫格已迁入灵境工坊 | ✅ v526 |
...
### 20.3 V26 编辑我的作品·历史全集（MW-01~03）
| # | 功能 | 实现 |
|---|---|---|
| MW-01 | 功能按键化 | creator-center 移除内嵌作品长列表（西游记/红楼梦卡片），「编辑我的作品」改为功能区功能按键 → `my-works.html`；创作页防滑动串扰（铁律 #9） |
| MW-02 | 历史作品全集页 | `my-works.html`（v526）：全部/已发布/草稿·待续写/审核中/已下架/角色·道具 6 筛选；数据 = 上传/连载历史（wid 与 work-editor WP_WORKS 对齐）+ CreatorStore.works() 真实记录同名合并；已发布=续写/收费/数据/预览，草稿=继续续写，审核=查质检，下架=申诉+改后重提；底部上传/创建双入口 |
| MW-03 | 工作台去重 | creator-center「灵境工坊·工作台」七宫格（编辑器/AI辅助/大纲/逐章/收费/封面/发布）整体迁入 `workshop.html` 创作工作台段（含 realname 门控）；创作页删除重复段 |
| 回归 | `scripts/test_v26_my_works.py` | **24/24 PASS**（8799 独立端口）；test_v23 47/47、test_v25 10/10、test_v20-o 创作段 20/20 同步修复（creator-store 公版种子失配 + 工作台迁移断言） |
